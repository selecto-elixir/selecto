defmodule Selecto.SimpleCteTest do
  ...
  describe "Selecto-powered CTE functionality" do
    ...
    test "build_recursive_cte_from_selecto logic works with mock SQL" do
      ...
      recursive_sql =
        "SELECT c.id, c.name, h.level + 1 FROM categories c JOIN hierarchy h ON c.parent_id = h.id WHERE h.level < $1"
      ...
      assert ["RECURSIVE ", cte_name, " AS (", base_part, " UNION ALL ", recursive_part, ")"] =
               cte_iodata
    ...
    test "CTE integration with parameter system works" do
      ...
      main_query = [
        "SELECT count(*) FROM active_users WHERE created_at > ",
        {:param, ~D[2024-01-01]}
      ]
    ...
    test "multiple CTE building logic works" do
      ...
      cte1_iodata = [
        "users_cte AS (SELECT id, name FROM users WHERE active = ",
        {:param, true},
        ")"
      ]
      ...
      cte2_iodata = [
        "posts_cte AS (SELECT id, title FROM posts WHERE created_at > ",
        {:param, ~D[2024-01-01]},
        ")"
      ]
