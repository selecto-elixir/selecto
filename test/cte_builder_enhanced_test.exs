defmodule Selecto.CteBuilderEnhancedTest do
  ...
  describe "build_recursive_cte/5 edge cases" do
    test "builds recursive CTE with complex parameter distribution" do
      ...
      {cte_iodata, combined_params} =
        Cte.build_recursive_cte(
          "hierarchy",
          base_iodata,
          base_params,
          recursive_iodata,
    ...
    test "handles recursive CTE with no parameters in base case" do
      ...
      recursive_iodata = [
        "SELECT c.id, c.name, p.level + 1 FROM categories c JOIN cte p ON c.parent_id = p.id WHERE p.level < ",
        {:param, 3}
      ]
    ...
    test "handles recursive CTE with no parameters in recursive case" do
      base_iodata = [
        "SELECT id, name, 0 as level FROM categories WHERE type = ",
        {:param, "root"}
      ]
  ...
  describe "build_with_clause/1 comprehensive tests" do
    ...
    test "preserves parameter order across multiple CTEs with mixed ordering" do
      ...
      cte1 =
        {["cte1 AS (SELECT * FROM t1 WHERE a = ", {:param, 1}, " AND b = ", {:param, 2}, ")"],
         [1, 2]}
      ...
      cte3 =
        {[
           "cte3 AS (SELECT * FROM t3 WHERE p = ",
           {:param, true},
           " AND q = ",
           {:param, false},
           " AND r = ",
  ...
  describe "integrate_ctes_with_query/3 comprehensive tests" do
    test "integrates multiple CTEs with parameterized main query" do
      ...
      cte2 =
        {["cte2 AS (SELECT user_id FROM posts WHERE status = ", {:param, "published"}, ")"],
         ["published"]}
      ...
      main_query = [
        ...
        "WHERE c1.id > ",
        {:param, 100},
        " GROUP BY c1.id ORDER BY post_count DESC LIMIT ",
        {:param, 50}
      ]
      ...
      {complete_query, combined_params} =
        Cte.integrate_ctes_with_query([cte1, cte2], main_query, main_params)
    ...
    test "handles main query with no parameters" do
      ...
      {complete_query, combined_params} =
        Cte.integrate_ctes_with_query([cte1], main_query, main_params)
    ...
    test "preserves complex nested iodata in integration" do
      ...
      {complete_query, combined_params} =
        Cte.integrate_ctes_with_query([nested_cte], nested_main, main_params)
  ...
  describe "end-to-end finalization" do
    test "complex CTE workflow produces valid SQL" do
      ...
      recursive_hierarchy = [
        ...
        "JOIN hierarchy h ON c.parent_id = h.id WHERE h.level < ",
        {:param, 5}
      ]

      {recursive_cte, recursive_params} =
        Cte.build_recursive_cte(
          "hierarchy",
          base_hierarchy,
          [],
      ...
      {users_cte, users_params} =
        Cte.build_cte("active_users", active_users_query, [true, ~D[2024-01-01]])
      ...
      {complete_query, combined_params} =
        Cte.integrate_ctes_with_query(all_ctes, main_query, main_params)
    ...
    test "parameter numbering is correct in complex scenarios" do
      ...
      cte1 =
        {[
           "cte1 AS (SELECT * FROM t1 WHERE a = ",
           {:param, "a1"},
           " AND b = ",
           {:param, "b1"},
           ")"
      ...
      {complete_query, combined_params} =
        Cte.integrate_ctes_with_query([cte1, cte2, cte3], main_query, main_params)
      ...
      # Should be "x2"
      assert String.contains?(final_sql, "$3")
      # Should be "p3"
      assert String.contains?(final_sql, "$4")
      # Should be "q3" 
      assert String.contains?(final_sql, "$5")
      # Should be "r3"
      assert String.contains?(final_sql, "$6")
      # Should be "main1"
      assert String.contains?(final_sql, "$7")
      # Should be "main2"
      assert String.contains?(final_sql, "$8")
