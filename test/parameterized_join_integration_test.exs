defmodule Selecto.ParameterizedJoinIntegrationTest do
  ...
  describe "process_parameterized_join/6" do
    ...
    test "processes parameterized join with all parameters", %{
    ...
    test "applies default values for missing optional parameters", %{
      ...
      # default
      assert param3.value == true
      # default
      assert param4.value == false
    end

    test "raises error for missing required parameters", %{
      join_config: join_config,
      queryable: queryable
    } do
      # No parameters provided, but category is required
      parameters = []
    ...
    test "raises error for parameter type mismatch", %{
      join_config: join_config,
      queryable: queryable
    } do
      parameters = [
        # Should be string for category
        {:integer, 123}
      ]
  ...
  describe "validate_parameters/2" do
    test "validates all parameter types correctly" do
      ...
      # converted to atom
      assert Enum.at(validated, 4).value == :featured
    end
  ...
  describe "resolve_parameterized_condition/2" do
    test "replaces parameter placeholders in join condition" do
      join_config = %{
        join_condition:
          "{join_alias}.category = $param_category AND {join_alias}.price >= $param_min_price"
      }
    ...
    test "handles different parameter types" do
      join_config = %{
        join_condition:
          "field1 = $param_str AND field2 = $param_int AND field3 = $param_bool AND field4 = $param_float"
      }
