defmodule Selecto.FieldResolverTest do
  ...
  describe "resolve_field/2" do
    ...
    test "resolves disambiguated field reference", %{selecto: selecto} do
      {:ok, field_info} =
        FieldResolver.resolve_field(selecto, {:disambiguated_field, "id", from: "posts"})
    ...
    test "resolves explicitly qualified field reference", %{selecto: selecto} do
      {:ok, field_info} =
        FieldResolver.resolve_field(selecto, {:qualified_field, "comments.content"})
    ...
    test "returns error for ambiguous field with suggestions", %{selecto: selecto} do
      ...
      selecto =
        put_in(selecto.config.source.columns, Map.delete(selecto.config.source.columns, :id))
  ...
  describe "get_available_fields/1" do
    test "returns all available fields from source, joins, and declared CTE columns", %{
      selecto: selecto
    } do
      ...
      # redacted
      refute Map.has_key?(fields, "email")
