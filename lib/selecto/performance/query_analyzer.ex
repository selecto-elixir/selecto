defmodule Selecto.Performance.QueryAnalyzer do
  ...
  defp analyze_plan(plan_data) when is_map(plan_data) do
    ...
    final_analysis = Map.merge(analysis, %{
  ...
  defp analyze_plan_node(node) when is_map(node) do
    ...
    metrics = if metrics.actual_time > 10.0 do
      %{metrics | slow_nodes: [%{
        type: node_type,
    ...
    child_metrics = case node["Plans"] do
  ...
  defp calculate_estimation_accuracy(%{actual_rows: actual, plan_rows: planned}) when planned > 0 do
  ...
  defp generate_suggestions(plan_metrics) do
    ...
    suggestions = if "Seq Scan" in plan_metrics.node_types do
    ...
    suggestions = suggestions ++ Enum.map(plan_metrics.slow_nodes, fn node ->
      "Slow #{node.type} operation on #{node.relation || "unknown table"} (#{node.time}ms)"
    end)
    
    # Check row estimation accuracy
    suggestions = case calculate_estimation_accuracy(plan_metrics) do
      :underestimated ->
  ...
  defp generate_index_recommendations(index_info) do
    ...
    recommendations = if index_info.seq_scans > 3 do
      ["Multiple sequential scans detected - review indexing strategy" | recommendations]
    else
      recommendations
    end
    
    recommendations = recommendations ++ Enum.map(index_info.missing, fn missing ->
  ...
  defp generate_comparison_recommendations(analysis1, analysis2) do
    ...
    recommendations = cond do
      time_diff > 50 ->
        ["Query 2 is significantly slower (#{Float.round(time_diff, 2)}ms difference)" | recommendations]
      time_diff < -50 ->
        ["Query 2 is significantly faster (#{Float.round(abs(time_diff), 2)}ms improvement)" | recommendations]
      true ->
        recommendations
    end
    ...
    recommendations = if added_nodes != [] do
      ["Query 2 introduces: #{Enum.join(added_nodes, ", ")}" | recommendations]
    else
      recommendations
    end
    
    recommendations = if removed_nodes != [] do
  ...
  defp extract_tables_from_query(selecto) do
    # Extract table names from the query structure
    tables = [selecto.source[:source_table]]
    
    # Add joined tables
    join_tables = selecto.joins
      |> Enum.map(fn {_alias, join_info} ->
        join_info[:source_table] || join_info[:table]
      end)
      |> Enum.filter(&(&1))
    
    Enum.uniq(tables ++ join_tables)
  end
