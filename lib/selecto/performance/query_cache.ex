defmodule Selecto.Performance.QueryCache do
  ...
  def init(opts) do
    ...
    stats_table = if config.track_stats do
    ...
    case lookup_cache(state.cache_table, cache_key) do
      {:ok, entry} ->
        ...
        if expired?(entry) do
          ...
          new_state = if state.config.eviction_policy == :lru do
  ...
  def handle_call({:invalidate, pattern}, _from, state) do
    ...
    new_state = %{state |
      item_count: max(0, state.item_count - count)
    }
  ...
  def handle_call(:stats, _from, state) do
    stats = if state.stats_table do
      :ets.tab2list(state.stats_table)
      |> Map.new()
      |> Map.merge(%{
        hit_rate: calculate_hit_rate(state.stats_table),
        avg_entry_size: if(state.item_count > 0, do: state.size_bytes / state.item_count, else: 0),
  ...
  def handle_call({:warmup, queries}, _from, state) do
    ...
    warmed = Enum.reduce(queries, 0, fn {selecto, result}, count ->
      cache_key = generate_key(selecto)
  ...
  def handle_info(:ttl_cleanup, state) do
    ...
    new_state = %{state |
      item_count: max(0, state.item_count - expired_count)
    }
  ...
  defp evict_lfu_entry(state) do
    ...
    case find_lfu_entry(state.cache_table) do
      ...
      {key, _entry} ->
        ...
        %{state |
          lru_list: List.delete(state.lru_list, key),
          item_count: state.item_count - 1
        }
    end
  end
  ...
  defp update_lru(state, key) do
    new_lru = [key | List.delete(state.lru_list, key)]
