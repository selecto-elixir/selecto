defmodule Selecto.Performance.Hooks do
      ...
      if options[:cache] do
        ...
        case Selecto.Performance.QueryCache.get(cache_key) do
          ...
          :miss ->
            ...
            run_hooks(:on_cache_miss, context)
        end
      else
        context
      end
    ...
    result =
      if context[:cache_hit] do
        ...
        if options[:cache] && context[:cache_key] && match?({:ok, _}, execution_result) do
          cache_opts =
            case options[:cache_ttl] do
              ttl when is_integer(ttl) -> [ttl: ttl]
              _ -> []
            end

          Selecto.Performance.QueryCache.put(
            context.cache_key,
            elem(execution_result, 1),
            cache_opts
          )
        end
  ...
  def install_default_hooks(options \\ []) do
    ...
    register(:after_execution, fn context ->
      if context.execution_time > slow_query_threshold do
        ...
        Logger.warning(
          "Slow query detected (#{context.execution_time}ms): #{LogSanitizer.sanitize_query(context.sql, context[:params] || [], max_length: 200)}"
        )
  ...
  defp format_analysis(analysis) do
    ...
    Slow Operations:
    #{Enum.map_join(analysis.slow_nodes, "\n", fn node -> "  - #{node.type} on #{node.relation || "unknown"} (#{node.time}ms)" end)}
    """
  end
end
