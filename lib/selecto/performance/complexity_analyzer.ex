defmodule Selecto.Performance.ComplexityAnalyzer do
  ...
  def analyze(selecto, opts \\ []) do
    max_score = opts[:max_complexity] || Application.get_env(:selecto, :max_complexity, @max_complexity_score)
    max_joins = opts[:max_joins] || Application.get_env(:selecto, :max_joins, @max_joins)
    max_in_size = opts[:max_in_clause_size] || Application.get_env(:selecto, :max_in_clause_size, @max_in_clause_size)
  ...
  defp finalize_analysis(analysis, max_score, opts) do
    ...
    analysis = if analysis.score > @warn_threshold && analysis.score <= max_score do
      %{analysis |
        warnings: analysis.warnings ++ [
    ...
    if should_block do
      ...
      analysis = if analysis.score > max_score && length(analysis.blocking_issues) == 0 do
        %{analysis |
          blocking_issues: analysis.blocking_issues ++ [
  ...
  defp count_joins(joins) when is_map(joins) do
    Enum.reduce(joins, 0, fn {_name, join_config}, acc ->
      nested_count = case Map.get(join_config, :joins) do
        nil -> 0
  ...
  defp count_leading_wildcard_like(filters) do
    Enum.count(filters, fn filter ->
      case filter do
        ...
        %{"comp" => comp, "value" => value} when comp in ["LIKE", "like", "ILIKE", "ilike"] and is_binary(value) ->
          String.starts_with?(value, "%")
  ...
  def format_summary(analysis) do
    ...
    lines = if length(analysis.blocking_issues) > 0 do
    ...
    lines = if length(analysis.warnings) > 0 do
      lines ++ ["  Warnings:"] ++
    ...
    lines = if length(analysis.recommendations) > 0 do
