defmodule Selecto.Performance.MetricsCollector do
  ...
  def handle_cast({:record_query, query_id, metrics}, state) do
    ...
    # Track slow queries
    new_slow_queries = if metrics.execution_time > state.config.slow_query_threshold do
    ...
    new_patterns = if state.config.track_patterns do
  ...
  def handle_cast({:complete_query, query_id, result_info}, state) do
    case :ets.lookup(state.ets_table, query_id) do
      [{^query_id, start_metrics}] ->
        ...
        complete_metrics = Map.merge(start_metrics, %{
        ...
        complete_metrics = if state.config.track_memory do
          Map.put(complete_metrics, :memory_used, calculate_memory_usage())
        else
          complete_metrics
        end
        
        complete_metrics = if state.config.track_cpu do
  ...
  def handle_call({:get_query_metrics, query_id}, _from, state) do
    metrics = case :ets.lookup(state.ets_table, query_id) do
  ...
  defp update_stats(stats, metrics) do
    %{stats |
      ...
      avg_execution_time: (stats.total_execution_time + metrics.execution_time) / (stats.total_queries + 1),
  ...
  defp add_slow_query(slow_queries, query_id, metrics) do
    ...
    [entry | slow_queries]
      ...
      |> Enum.take(100)  # Keep top 100 slow queries
  end
  ...
  defp calculate_stats_for_range(state, {:last, amount, unit}) do
    ...
    metrics = :ets.select(state.ets_table, [
      {
        {:"$1", %{started_at: :"$2"}},
        [{:>=, :"$2", cutoff}],
        [:"$_"]
  ...
  defp export_metrics_data(state, :csv, options) do
    ...
    rows = :ets.tab2list(state.ets_table)
