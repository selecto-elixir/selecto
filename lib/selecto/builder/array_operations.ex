defmodule Selecto.Builder.ArrayOperations do
  ...
  defp build_array_size_sql(%Spec{operation: op} = spec, params_list, selecto)
       when op in [:array_ndims, :array_dims] do
    column_sql = build_column_reference(spec.column, selecto)
  ...
  defp build_array_element_sql(%Spec{operation: op} = spec, params_list, selecto)
       when op in [:array_position, :array_positions] do
    column_sql = build_column_reference(spec.column, selecto)
    func_name = String.upcase(to_string(op))
  ...
  defp build_array_transform_sql(%Spec{operation: :array_to_string} = spec, params_list, selecto) do
    ...
    iodata =
      if spec.options[:null_string] do
