defmodule Selecto.Schema.Join do
  ...
  defp configure_adjacency_list(id, association, config, parent, from_source, queryable, name, depth_limit) do
    # Add custom columns for hierarchy navigation
    config = Map.put(config, :custom_columns, Map.get(config, :custom_columns, %{}) |> Map.put(
        "#{id}_path", %{
          name: "#{name} Path",
          select: build_hierarchy_path_sql(association.field, queryable.source_table, depth_limit),
          group_by_format: fn {a, _id}, _def -> a end,
          filterable: false
        }
      ) |> Map.put(
        "#{id}_level", %{
          name: "#{name} Level",
          select: build_hierarchy_level_sql(association.field, queryable.source_table, depth_limit),
          group_by_format: fn {a, _id}, _def -> a end,
          filterable: true
        }
      )
  ...
  defp configure_materialized_path(id, association, config, parent, from_source, queryable, name) do
    ...
    config = Map.put(config, :custom_columns, Map.get(config, :custom_columns, %{}) |> Map.put(
        "#{id}_depth", %{
          name: "#{name} Depth",
          select: "length(#{association.field}[#{path_field}]) - length(replace(#{association.field}[#{path_field}], '#{path_separator}', ''))",
          group_by_format: fn {a, _id}, _def -> a end,
          filterable: true
        }
      ) |> Map.put(
        "#{id}_ancestors", %{
          name: "#{name} Ancestors",
          select: "#{association.field}[#{path_field}]",  
          group_by_format: fn {a, _id}, _def -> a end,
          filterable: false
        }
      )
  ...
  defp configure_closure_table(id, association, config, parent, from_source, queryable, name) do
    ...
    # Add custom columns leveraging closure table
    config = Map.put(config, :custom_columns, Map.get(config, :custom_columns, %{}) |> Map.put(
        "#{id}_descendant_count", %{
          name: "#{name} Descendant Count",
          select: "count_descendants.count",  # Will need special handling in SQL builder
          join_table: closure_table,
          group_by_format: fn {a, _id}, _def -> a end,
          filterable: true
        }
      )
