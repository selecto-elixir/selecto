defmodule Selecto.Advanced.ValuesClause do
  ...
  defmodule Spec do
    ...
    """
    defstruct [
      # Unique identifier for the VALUES clause
      :id,
      # List of data rows (lists or maps)
      :data,
      # Column names (inferred from data or explicit)
      :columns,
      # Alias for the VALUES table
      :alias,
      # :list_of_lists, :list_of_maps
      :data_type,
      # Inferred data types for each column
      :column_types,
      # Boolean indicating if data has been validated
      :validated
    ]
  ...
  defp validate_map_consistency([first_row | rest]) when is_map(first_row) do
    ...
    if Enum.empty?(invalid_rows) do
      ...
      actual_keys =
        if is_map(invalid_row),
          do: Map.keys(invalid_row) |> Enum.map(&to_string/1) |> Enum.sort(),
          else: []
        ...
        if length(columns) == length(first_row) do
          ...
          {:error,
           %ValidationError{
             type: :inconsistent_columns,
             message:
               "Number of explicit columns (#{length(columns)}) doesn't match data columns (#{length(first_row)})",
