defmodule Selecto.Advanced.LateralJoin do
  ...
  defmodule Spec do
    ...
    """
    defstruct [
      # Unique identifier for the LATERAL join
      :id,
      # :left, :inner, :right, :full
      :join_type,
      # Function that builds the correlated subquery
      :subquery_builder,
      # Alternative: table function specification
      :table_function,
      # Alias for the LATERAL join results
      :alias,
  ...
  def create_lateral_join(join_type, subquery_builder, alias_name, _opts)
      when is_function(subquery_builder) do
    %Spec{
  ...
  def create_lateral_join(join_type, table_function, alias_name, _opts)
      when is_tuple(table_function) do
    %Spec{
      id: generate_lateral_id(alias_name),
      join_type: join_type,
  ...
  defp extract_refs_from_filter({_field, {:ref, ref_field}}) when is_binary(ref_field),
    do: [ref_field]

  defp extract_refs_from_filter({_field, {_op, {:ref, ref_field}}}) when is_binary(ref_field),
    do: [ref_field]
